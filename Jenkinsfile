    ...
    steps {
        ...
        timeout(time: 5, unit: 'MINUTES') {
            script {
                ...
                try {
                    // 1. Grab project key from scanner report
                    ...
                    withCredentials([string(credentialsId: 'Jenkins-SonarQubeServer1', variable: 'SONAR_TOKEN')]) {
                        ...
                        // Use basic authentication (-u) passing the token
                        def response = sh(
                            script: "curl -s -u \$SONAR_TOKEN: 'http://sonarqube:9000/api/measures/component?component=${projectKey}&metricKeys=${metricKeys}'", 
                            returnStdout: true
                        ).trim()
                    ...
                    echo "⚠️ Failed to parse SonarQube metrics: ${e.toString()}"
    ...
    steps {
        script {
            ...
            withCredentials([string(credentialsId: 'argocd-admin-password', variable: 'ARGO_PASS')]) {
                ...
                echo "🔑 Logging into ArgoCD configuration profile..."
                // Authenticate the local profile runner silently
                // 2. Pass the environment variable ($ARGO_PASS) to the shell command
                sh "argocd login ${argoServer} --username ${argoUser} --password \$ARGO_PASS --insecure 2>/dev/null"

                echo "☸️ Fetching clean application deployment metrics..."
                // 2>/dev/null explicitly strips out any "level=warning" text before it hits jq
